...
export class DomHandler {
  ...
  public static addMultipleClasses(element: any, className: string): void {
    if (element.classList) {
      ...
      styles.forEach((s) => element.classList.add(s));
    } else {
      const styles: string[] = className.split(' ');
      styles.forEach((s) => (element.className += ' ' + s));
    }
  }
  ...
  public static indexWithinGroup(element: any, attributeName: string): number {
    ...
    for (const child of children) {
      ...
      if (child.attributes && child.attributes[attributeName] && child.nodeType === 1) {
        num++;
      }
    }
    return -1;
  ...
  public static fadeIn(element, duration: number): void {
    ...
    const tick = () => {
      ...
      if (+opacity < 1 && window.requestAnimationFrame && requestAnimationFrame(tick)) {
        setTimeout(tick, 16);
      }
    };
    tick();
